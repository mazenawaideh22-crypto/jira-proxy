...
var pendingCodes = {};
const DEVELOPER_IDS = ['YOUR_FIGMA_USER_ID_HERE'];
const usageStore = {};
const FREE_LIMIT = 3;
...
function successPage(code, service) {
  // Uses history.replaceState to remove the callback URL + code from browser history
...
app.get('/usage', function(req, res) {
  ...
  var count = isDev ? 0 : (usageStore[userId] || 0);
  res.json({ count: count, limit: FREE_LIMIT, allowed: isDev || count < FREE_LIMIT, isPaid: false });
});
...
app.get('/auth/gitlab', function(req, res) {
  var url = 'https://gitlab.com/oauth/authorize' +
    ...
    '&response_type=code' +
    '&scope=' + encodeURIComponent('read_user read_api') +
    '&state=' + crypto.randomBytes(16).toString('hex');
  res.redirect(url);
});
...
app.get('/auth/linear/callback', async function(req, res) {
  var code = req.query.code;
  if (!code) return res.status(400).send('<h2>Error: No code from Linear</h2>');
  try {
    var body = 'client_id=' + encodeURIComponent(LINEAR_CLIENT_ID) +
      '&client_secret=' + encodeURIComponent(LINEAR_CLIENT_SECRET) +
      '&code=' + encodeURIComponent(code) +
      '&redirect_uri=' + encodeURIComponent(BASE_URL + '/auth/linear/callback') +
      '&grant_type=authorization_code';
    ...
    if (!tokenRes.data.access_token) return res.status(400).send('<h2>Linear error: ' + JSON.stringify(tokenRes.data) + '</h2>');
...
app.get('/tickets', async function(req, res) {
  ...
  try {
    if (provider === 'gitlab') {
      ...
      var glPath = projectId
        ? '/api/v4/projects/' + encodeURIComponent(projectId) + '/issues?state=opened&per_page=30'
        : '/api/v4/issues?scope=assigned_to_me&state=opened&per_page=30';
      ...
      var tickets = (glRes.data || []).map(function(issue) { return { id: '#' + issue.iid, title: issue.title, description: issue.description || 'No description' }; });
...
app.post('/analyze', async function(req, res) {
  ...
  if (!isDev) {
    ...
    if (count >= FREE_LIMIT) return res.json({ error: 'limit_reached' });
